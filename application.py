...
def handle_something(event):
    ...
    if event.message.type=='text':
        ...
        if detect(path)!= False:
            type = detect(path)
            os.remove(path)
            url = uploadImage('static/result.jpeg', _id+'.jpeg')
            item = ShowDogType(type)
            reply_message_arr = [ImageSendMessage(original_content_url=url, preview_image_url=url), call_dog_Data(item)]
            line_bot_api.reply_message(event.reply_token, reply_message_arr)
        else:
            os.remove(path)
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text = "不好意思，辨識不出狗狗的品種，請您重新上傳"))
